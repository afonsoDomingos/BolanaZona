...
exports.update = async (req, res) => {
  try {
    ...
    const filter = isSuperAdmin ? { _id: req.params.id } : { _id: req.params.id, manager: req.user._id };
    const oldSquad = await Squad.findOne(filter);
...
exports.remove = async (req, res) => {
  try {
    const isSuperAdmin = req.user.role === 'superadmin';
    const filter = isSuperAdmin ? { _id: req.params.id } : { _id: req.params.id, manager: req.user._id };
    const squad = await Squad.findOneAndDelete(filter);
